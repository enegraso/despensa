...
const ReplenishmentList = () => {
  ...
  const fetchData = async (term = '') => {
    ...
    try {
      ...
      const url = `/api/reports/replenishment${qs ? '?' + qs : ''}`;
      ...
      <header className="bg-white shadow-sm print-hide">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-gray-800">Listado de Reposición</h1>
          </div>
          <button
            onClick={handlePrint}
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No hay artículos para reponer</div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    ...
                    <th className="text-right px-6 py-3 font-medium text-gray-500">Stock Mínimo</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-3 text-right font-semibold text-red-600">{item.stock_actual}</td>
                      <td className="px-6 py-3 text-right text-gray-600">{item.stock_minimo}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
